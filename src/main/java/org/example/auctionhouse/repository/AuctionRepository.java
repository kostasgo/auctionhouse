...
public interface AuctionRepository extends JpaRepository<Auction, Long> {
    final static String SEARCH_AUCTIONS = "SELECT DISTINCT * FROM auction auc JOIN seller sel ON auc.seller_id=sel.id JOIN auction_category ON auc.id = auction_category.auction_id JOIN category cat ON auction_category.categories_id = cat.id WHERE auc.active = :active AND auc.name LIKE %:search% AND sel.user_id != :id AND cat.name LIKE %:category% AND auc.currently <= :max AND auc.country LIKE %:country% GROUP BY auc.id ORDER BY auc.id DESC LIMIT 9 OFFSET :offset ";
    ...
    final static String SEARCH_AUCTIONS_COUNT = "SELECT COUNT(distinct auc.id) FROM auction auc JOIN seller sel ON auc.seller_id=sel.id JOIN auction_category ON auc.id = auction_category.auction_id JOIN category cat ON auction_category.categories_id= cat.id WHERE auc.active = :active AND auc.name LIKE %:search% AND sel.user_id != :id AND cat.name LIKE %:category% AND auc.currently <= :max AND auc.country LIKE %:country% " ;
    ...
    final static String GET_USER_AUCTIONS = "SELECT * FROM auction auc INNER JOIN seller sel ON auc.seller_id=sel.id WHERE sel.user_id = :id ORDER BY auc.id DESC LIMIT 8 OFFSET :offset";
